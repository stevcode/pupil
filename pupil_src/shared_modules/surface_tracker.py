            ...
            if action == GLFW_RELEASE:
                ...
                for s in self.edit_surfaces:
                    if s.detected and s.defined:
                        for (vx, vy), i in zip(
                            s.ref_surface_to_img(np.array(surf_verts)), range(4)
                        ):
                            vx, vy = denormalize(
                                (vx, vy),
    ...
    def recent_events(self, events):
        ...
        for s in self.surfaces:
            ...
            if s.detected:
                s.gaze_on_srf = s.map_data_to_surface(
                    events.get("gaze", []), s.m_from_screen
                )
        ...
        for s in self.surfaces:
            if s.detected:
                datum = {
                    ...
                    "camera_pose_3d": s.camera_pose_3d.tolist()
                    if s.camera_pose_3d is not None
                    else None,
                }
        ...
        if self.running:
            self.button.status_text = "{}/{}".format(
                len([s for s in self.surfaces if s.detected]), len(self.surfaces)
            )
        else:
