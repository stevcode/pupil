...
class Reference_Surface(object):
    ...
    def _get_location(
        ...
        for m in filtered_markers:
            if (
                m["id"] in marker_by_id
                and m["perimeter"] < marker_by_id[m["id"]]["perimeter"]
            ):
                pass
            else:
                marker_by_id[m["id"]] = m
        ...
        if overlap and len(overlap) >= min(2, len(requested_ids)):
            ...
            xy_undistorted_normalized = self.g_pool.capture.intrinsics.unprojectPoints(
                xy, use_distortion=self.use_distortion
            )[:, :2]
            ...
            corners_undistored_space = cv2.perspectiveTransform(
                marker_corners_norm.reshape(-1, 1, 2), m_to_undistored_norm_space
            )
