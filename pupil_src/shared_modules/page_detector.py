...
from math_helper.quad_segmentation import sortCorners


# hacked detect_markers
# now it detects a single outmost contour/page instead
# "pages" container is just for compatibility with the marker detector

# @profile
def detect_pages(gray_img, draw_contours=False):
    edges = cv2.adaptiveThreshold(gray_img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 25, -5)

    _, contours, hierarchy = cv2.findContours(edges, mode=cv2.RETR_TREE, method=cv2.CHAIN_APPROX_SIMPLE,
